...
//conver the canvas with a black rectangle
ctx.fillStyle = 'rgba(0,0,0,1)';
//fillRect(startingX,startingY ,width,height);
...
/**
 *this class sets up a new particle with its own
...
function Particle(){
	...
	//random size for the particle between 20 & 40
...
function drawParticle(){
	...
	//blend theparticle with background
	...
	for (var t=0; t<particles.length; t++){
		...
		//if particle heads left off the left edge
		if(particles[t].x < -50) particles[t].x = 550;
		
		//if particle heads right off the right edge
		if(particles[t].x > 550 ) particles[t].x = -50;
		
		//if particle heads top off the top edge
		if(particles[t].y < -50) particles[t].y = 550;
		
		//if particle heads bottom off the bottom edge
